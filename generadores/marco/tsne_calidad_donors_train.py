...
idx_donor = rng.choice(len(donor_train_raw), size=380, replace=False)  # muestreamos, para comparar tamaños parecidos a validation
...
combined = np.concatenate([donor_train_scaled.reshape(n_real, -1), synth_sample.reshape(n_real, -1)], axis=0)
...
ax.scatter(real_2d[:, 0], real_2d[:, 1], c="red", alpha=0.5, s=15, label="donor_train (muestra, real)")
ax.scatter(synth_2d[:, 0], synth_2d[:, 1], c="blue", alpha=0.3, s=15, label="sintetico (sin calibrar)")
ax.set_title("t-SNE -- donor_train real vs. sintetico (sin calibrar)")
ax.legend()
plt.tight_layout()
plt.savefig("tsne_donor_train_vs_sintetico.png", dpi=120)
print("Guardado: tsne_donor_train_vs_sintetico.png")

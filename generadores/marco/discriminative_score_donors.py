...
X = np.concatenate([donor_val_scaled.reshape(n_real, -1), synth_sample.reshape(n_real, -1)], axis=0)
...
axes[0].set_title("Matriz de confusion (donor_validation)")
for i in range(2):
    for j in range(2):
...
axes[1].legend()
plt.tight_layout()
plt.savefig("clasificador_donor_vs_sintetico.png", dpi=120)
print("Guardado: clasificador_donor_vs_sintetico.png")

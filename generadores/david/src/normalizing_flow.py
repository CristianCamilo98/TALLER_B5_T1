...
class TrainingConfig:
    ...
    gradient_norm_limit: float = 25.0
    patience: int = 200
...
class AffineCoupling:
    def __init__(self, *, mask: np.ndarray, config: FlowConfig, rng: np.random.Generator) -> None:
        ...
        self.scale_bound = float(config.scale_bound)
    ...
    def forward(self, values: np.ndarray, *, cache: bool = False) -> Any:
        ...
        scale = self.scale_bound * np.tanh(raw_scale) * self.inverse_mask
    ...
    def backward(
        ...
        grad_raw_scale = grad_scale * self.scale_bound * (1.0 - np.tanh(raw_scale) ** 2)
...
def _limit_gradients(grads: dict[str, np.ndarray], max_norm: float) -> float:
...
def train_real_nvp(
    ...
    *,
    ...
    if validation_windows is None:
        ...
        train_values = values
        validation_values = flatten_windows(validation_windows)
    ...
    for epoch in range(1, training_config.epochs + 1):
        ...
        for start in range(0, len(order), training_config.batch_size):
            ...
            grad_norms.append(_limit_gradients(grads, training_config.gradient_norm_limit))
